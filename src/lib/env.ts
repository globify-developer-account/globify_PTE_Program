...
export const env = {
  ...
  ai: {
    ...
    /** Optional gateway or proxy for AI_API_KEY requests. Empty means api.anthropic.com. */
    baseUrl: str('AI_BASE_URL'),
    /** Key used only by the question generator script, kept separate from the scoring key. */
    contentApiKey: str('CONTENT_AI_API_KEY'),
    /** Claude Platform on AWS — read for whichever key above is an AWS Console key. */
    aws: {
