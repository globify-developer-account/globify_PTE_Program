...
import { QuestionSearch } from '@/components/practice/question-search'
...
export default async function DashboardPage() {
  ...
  const [history, minutesToday, counts, classes, recentMocks, openSession, recommendations] =
    await Promise.all([
      getScoreHistory(user.id, 30),
      minutesPracticedToday(user.id),
      getPracticeCounts(user.id, timezone),
      getUpcomingClasses(user.id, 4),
  ...
  const daysToTest = profile?.preferredTestDate
    ? daysBetween(new Date(), profile.preferredTestDate)
    : null
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Jump straight to a task by reference or by a phrase from its text. */}
      <QuestionSearch />

      {/* Study stats + class schedule */}
      <div className="grid gap-5 lg:grid-cols-2">
        <StudyStats
          counts={counts}
          examDate={profile?.preferredTestDate ?? null}
          daysToTest={daysToTest}
        />
        <UpcomingClasses classes={classes} timezone={timezone} />
      </div>

      {/* AI study tips */}
      <section aria-labelledby="study-tips">
        <div className="mb-3 flex items-center justify-between gap-4">
          <h3 id="study-tips" className="text-base font-semibold text-navy-900">
            AI study tips
          </h3>
          ...
          <Card className="lg:col-span-2">
            ...
            <CardBody>
              {entitlements.isPremium ? (
                ...
                <LockedTips count={recommendations.length} />
              )}
            </CardBody>
          </Card>
        </div>
...
/**
 * The upgrade surface for personalised tips.
 *
 * It states how many recommendations are waiting and what they are drawn from,
 * rather than blurring placeholder text — a real number is both more honest
 * and more persuasive than a fake one.
 */
function LockedTips({ count }: { count: number }) {
  return (
    <div className="flex flex-col items-start gap-4 rounded-xl border border-brand-100 bg-brand-50/60 p-5 sm:flex-row sm:items-center">
      <span className="grid size-11 shrink-0 place-items-center rounded-xl bg-white text-brand-600">
        <Lock className="size-5" aria-hidden />
      </span>
      <div className="min-w-0 flex-1">
        <p className="text-sm font-semibold text-navy-900">
          {count > 0
            ? `${pluralize(count, 'recommendation')} ready from your results`
            : 'Personalised recommendations'}
        </p>
        <p className="mt-1 text-sm leading-relaxed text-ink-600">
          Premium shows which task types are costing you the most marks, the accuracy you need on
          each to reach your target, and where you stand today.
        </p>
      </div>
